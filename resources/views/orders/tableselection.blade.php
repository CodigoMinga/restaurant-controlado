...
@section('content')
    <div class="d-flex flex-column h-100">
        ...
        <ul class="nav nav-pills mb-3 pl-4 pr-4" id="pills-tab" role="tablist">
            @foreach ($tabletypes as $tabletype)
                @if(count($tabletype->tables)>0)
                <li class="nav-item" role="presentation">
                    <a class="nav-link {{ $loop->index==0 ? 'active' : '' }}" data-toggle="pill" href="#pills-{{$tabletype->id}}" role="tab"  aria-selected="{{ $loop->index==0 ? 'true' : 'false' }}">{{$tabletype->name}}</a>
                </li>
                @endif
            @endforeach
        </ul>
